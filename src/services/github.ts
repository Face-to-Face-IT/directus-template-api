/* eslint-disable @typescript-eslint/no-explicit-any -- GitHub API responses are dynamic */
  ...
  async function getTemplates(customUrl?: string): Promise<TemplateInfo[]> {
    ...
    const templateInfos = await Promise.all(
      directories.map(async (dir): Promise<TemplateInfo> => {
        ...
        const packageJsonPath = joinPath(repo.path || '', dirPath, 'package.json')
        let name = dirName
        let description: string | undefined
        ...
        try {
          ...
          if (error.status !== 404) {
            ux.warn(`Could not fetch or parse package.json for template "${dirName}": ${error.message}`)
          }
        }
